...
class Simplex:
    ...
    def get_coluna_pivo_index(self, linha_pivo_index:int) -> int:
        resultados = {}
        for coluna in range(len(self.table[0])):
            if self.table[linha_pivo_index][coluna] != 0:
                if self.table[0][coluna] != 0:
                    divisao = abs(self.table[0][coluna] / self.table[linha_pivo_index][coluna])
                    resultados[coluna] = divisao
        index = min(resultados, key=resultados.get)

        return index
    
    def get_linha_pivo_index(self)->int:
        resultados = {}
        for linha in range(len(self.table)):
            if linha > 0:
                if self.table[linha][-1]<0:
                    resultados[linha] = self.table[linha][-1]
        index = min(resultados, key=resultados.get)
    ...
    def caucular(self):
        ...
        coluna_pivo_index = self.get_coluna_pivo_index(linha_pivo_index)
