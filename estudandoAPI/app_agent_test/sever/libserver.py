...
class Message:
    ...
    def _set_selector_events_mask(self, mode):
        ...
        if mode == "r":
            ...
            loger.logger.error("Invalid events mask")
    ...
    def _write(self):
        if self._send_buffer:
            ...
            try:
                ...
                loger.logger.debug("Sending msg to client")
            except BlockingIOError:
                # Resource temporarily unavailable (errno EWOULDBLOCK)
                loger.logger.error("it's not posible to send to client")
                pass
            else:
                ...
                if sent and not self._send_buffer:
                    self.close()
                    loger.logger.debug("closing the Connection to the client")
    ...
    def close(self):
        ...
        try:
            ...
            loger.logger.error("Error: selector.unregister() exception for client")

        try:
            self.sock.close()
        except OSError as e:
            print(f"Error: socket.close() exception for {self.addr}: {e!r}")
            loger.logger.error("Error: socket.close() exception for client")
        finally:
    ...
    def process_request(self):
        ...
        if self.jsonheader["content-type"] == "text/json":
            ...
            loger.logger.info("Received 'Binary or unknown content-type' request from client")
